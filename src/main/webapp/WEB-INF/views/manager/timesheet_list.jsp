...
<body>
	...
	<table>
		...
		<tr>
			...
			<td><a
				href="/timesheet/toggleAuthority/<%=timesheet.getTask_id()%>"><%=status ? "Approved" : "Pending"%></a></td>
			<td><input type="hidden" id="timesheetId_<%=sNo%>"
				value="<%=timesheet.getTimesheet_id()%>" /> <input type="button"
				class="button-delete" value="Delete"
				onclick="deleteTimesheet(<%=sNo%>);" /></td>
		</tr>
		<%
		}
		%>
